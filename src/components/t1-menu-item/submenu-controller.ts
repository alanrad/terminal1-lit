...
import { type HasSlotController } from '@utils/slot';
import { html } from 'lit';
import type { ReactiveController, ReactiveControllerHost } from 'lit';
import type T1MenuItem from './menu-item.component';
...
export class SubmenuController implements ReactiveController {
  private host: ReactiveControllerHost & T1MenuItem;
  private popupRef: Ref<T1Popup> = createRef();
  ...
  constructor(host: ReactiveControllerHost & T1MenuItem, hasSlotController: HasSlotController) {
  ...
  private addListeners() {
    ...
    if (!this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.addEventListener('t1-reposition', this.handlePopupReposition);
  ...
  private removeListeners() {
    ...
    if (this.isPopupConnected) {
      if (this.popupRef.value) {
        ...
        this.popupRef.value.removeEventListener('t1-reposition', this.handlePopupReposition);
  ...
  private handleSubmenuEntry(event: KeyboardEvent) {
    ...
    for (const elt of submenuSlot.assignedElements()) {
      menuItems = elt.querySelectorAll("t1-menu-item, [role^='menuitem']");
