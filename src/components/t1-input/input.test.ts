...
test('disabled attribute disables the inner input', async () => {
  el.disabled = true;
  await el.updateComplete;
...
test('placeholder attribute is forwarded', async () => {
...
test('required attribute is forwarded', async () => {
  el.required = true;
  await el.updateComplete;
  const input = el.shadowRoot!.querySelector<HTMLInputElement>('.input__control')!;
...
test('type attribute is forwarded', async () => {
  el.type = 'email';
  await el.updateComplete;
  const input = el.shadowRoot!.querySelector<HTMLInputElement>('.input__control')!;
...
test('size attribute adds size class', async () => {
  el.size = 'large';
  await el.updateComplete;
...
test('filled variant adds filled class', async () => {
  el.filled = true;
  await el.updateComplete;
  const base = el.shadowRoot!.querySelector('[part="base"]')!;
...
test('pill modifier adds pill class', async () => {
  el.pill = true;
  await el.updateComplete;
  const base = el.shadowRoot!.querySelector('[part="base"]')!;
...
test('label attribute shows label', async () => {
  el.label = 'My Label';
  await el.updateComplete;
...
test('help text attribute shows help text', async () => {
...
test('clearable shows clear button when has value', async () => {
  el.clearable = true;
  el.value = 'hello';
  await el.updateComplete;
...
test('clearable hides clear button when value is empty', async () => {
  el.clearable = true;
  el.value = '';
  await el.updateComplete;
  const clearBtn = el.shadowRoot!.querySelector('[part="clear-button"]');
...
test('password-toggle shows toggle button', async () => {
  el.type = 'password';
...
test('t1-focus event fires on focus', async () => {
  await el.updateComplete;
  let fired = false;
...
test('t1-blur event fires on blur', async () => {
  await el.updateComplete;
  let fired = false;
...
test('t1-input event fires on input', async () => {
  await el.updateComplete;
  let fired = false;
...
test('t1-change event fires on change', async () => {
  await el.updateComplete;
  let fired = false;
...
test('t1-clear event fires on clear button click and resets value', async () => {
  el.clearable = true;
  el.value = 'hello';
  await el.updateComplete;
...
test('focus() method focuses the input', async () => {
  await el.updateComplete;
  let focused = false;
  const input = el.shadowRoot!.querySelector<HTMLInputElement>('.input__control')!;
...
test('disabled prevents clear button from showing', async () => {
  el.clearable = true;
  el.disabled = true;
  el.value = 'hello';
  await el.updateComplete;
...
test('readonly attribute is forwarded to inner input', async () => {
  el.readonly = true;
  await el.updateComplete;
  const input = el.shadowRoot!.querySelector<HTMLInputElement>('.input__control')!;
...
test('checkValidity returns true for a valid input', async () => {
  await el.updateComplete;
  expect(el.checkValidity()).toBe(true);
});

test('reportValidity returns true for a valid input', async () => {
  await el.updateComplete;
