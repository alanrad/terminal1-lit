...
test('defaults to circle shape', async () => {
...
test('base part has role="img" and aria-label', async () => {
...
test('shows icon fallback when no image or initials provided', async () => {
  const el = createElement({ label: 'Avatar' });
  await el.updateComplete;
...
(['circle', 'square', 'rounded'] as const).forEach((shape) => {
  test(`applies avatar--${shape} class for shape="${shape}"`, async () => {
...
test('renders initials part when initials are set', async () => {
...
test('does not render icon when initials are set', async () => {
  const el = createElement({ initials: 'AB', label: 'Avatar' });
  await el.updateComplete;
...
test('renders image part with src when image is set', async () => {
...
test('hides initials when image is provided', async () => {
  const src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
...
test('hides icon when image is provided', async () => {
  const src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  const el = createElement({ image: src, label: 'Avatar' });
  await el.updateComplete;
...
test('emits t1-error and falls back to fallback content when image fails', async () => {
  const el = createElement({ label: 'Avatar' });
  await el.updateComplete;
...
test('resets error state when a new image is set', async () => {
  const src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  const el = createElement({ label: 'Avatar' });
  await el.updateComplete;
...
test('loading attribute is forwarded to the img element', async () => {
  const src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  const el = createElement({ image: src, label: 'Avatar', loading: 'lazy' });
  await el.updateComplete;

  const img = el.shadowRoot!.querySelector<HTMLImageElement>('[part~="image"]')!;
  expect(img).not.toBeNull();
  expect(img.getAttribute('loading')).toBe('lazy');
});

test('accepts slotted content in the icon slot', async () => {
  const el = document.createElement('t1-avatar') as unknown as T1AvatarEl;
