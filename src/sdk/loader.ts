...
const CDN_BASE =
	((globalThis as Record<string, unknown>).__WIDGET_CDN__ as
		| string
		| undefined) ?? import.meta.url.replace(/\/sdk\/loader\.js.*$/, "");
...
const WIDGET_REGISTRY: Record<string, string> = {
	...
	"search-property-widget": "/widgets/search-property-widget.js",
};
...
async function loadWidget(tagName: string): Promise<void> {
	...
	if (!path) {
		console.warn(`[widgets] Unknown widget: "${tagName}"`);
		return;
	}
...
async function autoLoad(): Promise<void> {
	...
	const explicit = loaderScript?.dataset.widgets
		?.split(",")
		.map((s) => s.trim())
		.filter(Boolean);
